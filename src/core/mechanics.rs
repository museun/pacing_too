...
impl Simulation {
    const FLAVOR_TASKS: &[(&str, Duration)] = &[
        (
    ...
    pub fn tick(&mut self, rng: &mut Rand) {
    ...
    fn dequeue(&mut self, rng: &mut Rand) {
    ...
    fn complete_act(&mut self, rng: &mut Rand) {
    ...
    fn complete_quest(&mut self, rng: &mut Rand) {
        self.player
            .quest_book
            .quest
    ...
    fn cinematic(&mut self, rng: &mut Rand) {
        trait Queue {
            fn enqueue(&mut self, task: Task, rng: &mut Rand);
        }
...
impl Stats {
    ...
    pub fn iter(&self) -> impl ExactSizeIterator<Item = &(Stat, usize)> + use<'_> {
        self.values.iter()
    }
...
impl QuestBook {
    ...
    pub fn quests(&self) -> impl ExactSizeIterator<Item = &str> {
        self.quests.iter().map(|s| &**s)
    }

    pub fn completed_quests(&self) -> impl ExactSizeIterator<Item = &str> {
...
impl SpellBook {
    ...
    pub fn iter(&self) -> impl ExactSizeIterator<Item = (&str, i32)> {
        self.spells
            .iter()
...
impl Inventory {
    ...
    pub fn items(&self) -> impl ExactSizeIterator<Item = (&String, &usize)> {
        self.items
            .iter()
...
impl Equipment {
    ...
    pub fn iter(&self) -> impl ExactSizeIterator<Item = (config::Equipment, &str)> {
...
impl Player {
    ...
    pub fn level_up(&mut self, rng: &mut Rand) {
        self.level += 1;

        let mut adjust = |n| n / 3 + 1 + rng.below(4);
        for (amount, stat) in [
    ...
    fn choose_stat(&mut self, rng: &mut Rand) {
    ...
    fn choose_spell(&mut self, rng: &mut Rand) {
    ...
    fn choose_equipment(&mut self, rng: &mut Rand) {
    ...
    fn choose_item(&mut self, rng: &mut Rand) {
        self.inventory.add_item(special_item(rng), 1);
    }
}

fn special_item(rng: &mut Rand) -> String {
    format!(
        "{} of {}",
...
fn interesting_item(rng: &mut Rand) -> String {
    format!(
        "{} {}",
...
fn boring_item(rng: &mut Rand) -> &'static str {
    config::BORING_ITEMS.choice(rng)
}

fn impressive_npc(rng: &mut Rand) -> String {
...
fn unnamed_monster(level: usize, attempts: usize, rng: &mut Rand) -> config::Monster {
...
impl StatsBuilder {
    ...
    pub fn roll(&mut self, rng: &mut Rand) -> Stats {
        static MAX: usize = config::PRIME_STATS.len();
